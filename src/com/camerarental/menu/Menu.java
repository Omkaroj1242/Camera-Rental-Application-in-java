package com.camerarental.menu;

import java.util.ArrayList;
import java.util.Scanner;

import com.camerarental.entity.Camera;
import com.camerarental.main.Main;
import com.camerarental.mycameras.MyCamera;
import com.camerarental.wallet.MyWallet;
...
public class Menu {
	...
	Main main = new Main();
	ArrayList<Camera> cameras = main.cameras;
	...
	public void rentCamera() {
		...
		for(int i=0;i<cameras.size(); i++) {
			if(id == cameras.get(i).getId()) {
				if( wallet.getMyBalance() >= cameras.get(i).getPricePerDay() ) {
					wallet.debitAmount(cameras.get(i).getPricePerDay());
					cameras.get(i).setStatus("Rented");
					cam.myCamera.add(cameras.get(i));
					System.out.println("YOUR TRANSACTION FOR CAMERA - "+cameras.get(i).getBrand()+" "+cameras.get(i).getModel()+" WITH RENT INR."+cameras.get(i).getPricePerDay()+" HAS SUCCESSFULLY COMPLETED.");
					showCameras();
				}else {
	...
	public void wallet() {
		...
		System.out.println("YOUR CURRENT WALLET BALANCE IS - INR."+wallet.getMyBalance());
		...
		if(input == 1) {
			...
			System.out.println("YOUR WALLET BALANCE UPDATED SUCCESSFULLY. CURRENT WALLET BALANCE - INR."+wallet.getMyBalance());
		}
	}
